...
class Type_aware_MultiHeadedAttention(nn.Module):
    def __init__(self, h, d_model, dropout=0.1):
        """Take in model size and number of heads."""
        super(Type_aware_MultiHeadedAttention, self).__init__()
        assert d_model % h == 0
        # We assume d_v always equals d_k
        self.d_k = d_model // h
        self.h = h
    ...
    def forward(self, query, key, value, mask=None):
        ...
        """Implements Figure 2"""
        if mask is not None:
            # Same mask applied to all h heads and max_label_num indice.
            # batch_size, event_type_num,1(heads),1(query_num),max_label_num
        ...
        # 1) Do all the linear projections in batch from d_model => h x d_k
        query, key, value = [
            l(x).view(nbatches,ntypes, -1, self.h, self.d_k).transpose(2, 3)
            for l, x in zip(self.linears, (query, key, value))
        ]
        ...
        x = x.transpose(2, 3).contiguous() \
            .view(nbatches,ntypes, -1, self.h * self.d_k)
...
def get_transformer_encoder(num_layers, hidden_size, ff_size=2048, num_att_heads=8, dropout=0.1):
    dcopy = copy.deepcopy
